...
import static org.cricket.hawkeye.codegen.fetcher.AbstractFetcherMethodImpl.ENABLED;
 ...
 */
@ScanMe(true)
public class LikeMethodImpl extends AbstractFetcherMethodImpl {
    
    public LikeMethodImpl() {
    }
    
    @Override
    public boolean fieldImplements(Field field) {
        HQLGenerate hqlg = field.getAnnotation(HQLGenerate.class);
        boolean isImplemented = hqlg.comparision().like();
    ...
    public IFetcherMethodImpl create(SourceVO sourceVO) throws HawkEyeCodeTemplateException {
        ...
        fetcherTempl.setOwnerLikeImpl(ownsLikeImpl ? ENABLED : DISABLED);
